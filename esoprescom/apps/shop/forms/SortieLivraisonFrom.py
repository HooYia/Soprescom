...
class SortieLivraisonForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['date', 'ol', 'ccial', 'fact', 'bdc', 'client', 'reference', 'serial_number', 'nature', 'designation', 'qte_dde', 'stock_initial', 'observation']
        widgets = {
            ...
            'serial_number': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Enter Serail Number', 'required': 'required'}),
            'nature': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Enter Nature', 'required': 'required'}),
