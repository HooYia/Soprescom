...
class checkoutAddressForm(forms.ModelForm):
    ADDRESS_TYPE_CHOICES = [('billing','Billing'),
                          ('shipping','Shipping')]
    email = forms.EmailField(widget=forms.EmailInput(attrs={'class': 'form-control custom-text-input'}))
    ...
    class Meta:
        ...
        widgets = {
            ...
            'more_details' : forms.TextInput(attrs={'class': 'form-control custom-text-input'}),
            #'author' : forms.TextInput(attrs={'class': 'form-control custom-text-input'}),
